...
bool GlobalContext::createMoveHook(float inputSampleTime, csgo::UserCmd* cmd)
{
    ...
    features->misc.handleBotState(*memory, getEngineInterfaces());
...
void GlobalContext::fireGameEventCallback(csgo::GameEventPOD* eventPointer)
{
    ...
    case fnv::hash(csgo::player_death):
        features->misc.handleBotzEvents(*memory, getEngineInterfaces(), event, ClientInterfaces { retSpoofGadgets->client, * clientInterfaces },0);
        break;
    ...
    case fnv::hash(csgo::bullet_impact):
        ...
        break;
    case fnv::hash(csgo::bomb_planted):
        features->misc.handleBotzEvents(*memory, getEngineInterfaces(), event, ClientInterfaces { retSpoofGadgets->client, * clientInterfaces },3);
        break;
    case fnv::hash(csgo::player_radio):
        features->misc.handleBotzEvents(*memory, getEngineInterfaces(), event, ClientInterfaces { retSpoofGadgets->client, * clientInterfaces },6);
        break;
    case fnv::hash(csgo::weapon_fire):
        features->misc.handleBotzEvents(*memory, getEngineInterfaces(), event, ClientInterfaces { retSpoofGadgets->client, * clientInterfaces }, 12);
        break;
...
void GlobalContext::renderFrame()
{
    ...
    if (const auto& displaySize = ImGui::GetIO().DisplaySize; displaySize.x > 0.0f && displaySize.y > 0.0f) {
        features->misc.aimAtEvent(*memory,getEngineInterfaces());
